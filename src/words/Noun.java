...
public class Noun extends Word {
	...
	public boolean isUsedWithArticle(boolean isPlural) {
		boolean canBeArticle = ((isPlural ? SentenceGenerator.pluralArticle : SentenceGenerator.singularArticle)
				& flag) > 0;
		boolean canBeNoArticle = ((isPlural ? SentenceGenerator.pluralNoArticle : SentenceGenerator.singularNoArticle)
				& flag) > 0;
		if (canBeArticle && canBeNoArticle)
			return Math.random() > 0.5;
		return canBeArticle;
	}
}
